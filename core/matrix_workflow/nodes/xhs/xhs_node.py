...
class XhsNode(BaseNode):
    ...
    def _run(self) -> NodeRunResult: #  运行节点
        try:
            ...
            if self.operate == OperateEnums.COLLECT.value or self.operate == OperateEnums.SEARCH.value or self.operate == OperateEnums.COLLECT_REPLY.value or self.operate == OperateEnums.REPLY_COMMENT.value: # 采集
                result = start_script_by_type(self.device_id, self.operate, task_json, soft_type=4)
                if len(result) > 0:
                    collect_result = {}
                    if self.operate == OperateEnums.REPLY_COMMENT.value:
                        collect_result["reply_result"] = result
                    else:
        ...
        try:
            ...
            if node_data:
                if self.operate == OperateEnums.POST.value:
                    ...
                    task_json['reply_data'] = self.variable_pool.get(("run_outputs", node_data["reply_data"][0],node_data["reply_data"][1]))
                    self.result.inputs[node_data["reply_data"][1]]= task_json['reply_data']
